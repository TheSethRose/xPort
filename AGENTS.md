...
- **Debug dashboard data:** The dashboard is tweet-first. The default Tweets tab pages through all stored tweets from Postgres via daemon `/stored-tweets`, supplemented by related current-session capture event statuses from extension storage. Live Events is current-session only. Debug, transport, parser, and settings surfaces live in secondary tabs. Stored tweets and capture events should keep auto-refresh and auto-scroll controls. Long-list filters, including transcription status, should use searchable dropdowns; open filter dropdowns count as active editing, so auto-refresh may fetch in the background but must not rebuild the visible filter controls until the dropdown closes.
