...
export function extractTweets(endpoint, data) {
  ...
  if (!instructions || !Array.isArray(instructions)) {
    ...
    console.warn(`[XPort:parser] No instructions found for ${endpoint} | top-level keys: [${topKeys.join(', ')}]`);
    return [];
  }
  ...
  if (skippedEntries > 0 || Object.keys(skippedTypes).length > 0) {
    console.log(`[XPort:parser] ${endpoint}: ${tweets.length} tweets extracted, ${skippedEntries} entries skipped | skipped types: ${JSON.stringify(skippedTypes)}`);
  }
  ...
  if (path) {
    ...
    for (let i = 0; i < path.length; i++) {
      if (current == null || typeof current !== 'object') {
        ...
        console.warn(`[XPort:parser] Path broke for ${endpoint} at step ${i} ("${path[i]}") | value is: ${typeof current} | parent keys: [${Object.keys(parent || {}).join(', ')}]`);
        break;
      }
      if (current[path[i]] === undefined) {
        console.warn(`[XPort:parser] Path broke for ${endpoint} at step ${i} ("${path[i]}") | key not found | available keys: [${Object.keys(current).join(', ')}]`);
        break;
      }
      current = current[path[i]];
    }
  } else {
    console.log(`[XPort:parser] Unknown endpoint "${endpoint}", using recursive fallback`);
  }

  // Generic fallback: recursively search for an instructions array
  const fallback = findInstructionsRecursive(data, 5);
  if (fallback) {
    console.log(`[XPort:parser] Recursive fallback found instructions for ${endpoint}`);
  }
  return fallback;
}
...
function extractTweetsFromEntry(entry, skippedTypes) {
  ...
  if (!skippedTypes['_logged_' + entryType]) {
    ...
    console.warn(`[XPort:parser] Unknown entry type: "${entryType}" | keys: [${Object.keys(content).join(', ')}]`);
  }
...
function extractFromItemContent(itemContent) {
  ...
  if (itemContent.itemType !== 'TimelineTweet' && itemContent.__typename !== 'TimelineTweet') {
    ...
    if (!extractFromItemContent._seenTypes.has(itemType)) {
      ...
      console.log(`[XPort:parser] Skipping non-tweet itemType: "${itemType}"`);
    }
    return null;
  }
  ...
  if (!tweetResults?.result) {
    console.warn('[XPort:parser] TimelineTweet has no tweet_results.result | keys:', Object.keys(itemContent).join(', '));
    return null;
  }
...
function unwrapTweetResult(result) {
  ...
  if (result.legacy && result.core) {
    console.warn(`[XPort:parser] Unknown tweet __typename "${typename}", attempting extraction (has legacy+core)`);
    return result;
  }

  console.warn(`[XPort:parser] Unhandled tweet result __typename: "${typename}" | keys: [${Object.keys(result).join(', ')}]`);
  return null;
}
...
export function normalizeTweet(raw) {
  ...
  if (!legacy) {
    console.warn('[XPort:parser] Tweet missing legacy | keys:', Object.keys(raw).join(', '));
    return null;
  }
  ...
  if (!userResult) {
    console.warn(`[XPort:parser] No user data at raw.core.user_results.result | raw.core keys: [${Object.keys(raw.core || {}).join(', ')}]`);
  } else if (!userCore?.screen_name && !userLegacy?.screen_name) {
    console.warn(`[XPort:parser] No screen_name found | userResult keys: [${Object.keys(userResult).join(', ')}] | userResult.core keys: [${Object.keys(userCore || {}).join(', ')}] | userResult.legacy keys: [${Object.keys(userLegacy || {}).join(', ')}]`);
  }
...
export function extractMedia(legacy) {
  ...
  return mediaList.map(m => {
    ...
    if (m.type === 'photo') {
      ...
      console.warn(`[XPort:parser] Unknown media type: "${m.type}" | keys: [${Object.keys(m).join(', ')}]`);
    }
